...
class ExperimentCategory extends Model
{
    ...
    {
        return $this->hasMany(Experiment::class, 'experiment_category_id');
    }
     ...
     */
    public static function subtreeExperimentCounts(Collection $all, bool $publishedOnly = false): array
    {
        ...
        $direct = $query
            ->selectRaw('experiment_category_id, COUNT(*) as aggregate')
            ->whereNotNull('experiment_category_id')
            ->groupBy('experiment_category_id')
            ->pluck('aggregate', 'experiment_category_id')
            ->map(fn ($c) => (int) $c)
            ->all();
    ...
    public function breadcrumbItems(): array
    {
        return $this->ancestorChain()->map(fn (self $ancestor) => [
            'label' => $ancestor->name,
            'url' => route('experiments.category', $ancestor),
        ])->push([
            'label' => $this->name,
            'url' => null,
        ])->all();
