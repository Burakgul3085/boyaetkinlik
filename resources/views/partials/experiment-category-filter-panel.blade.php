...
@endphp
<aside
    class="exp-filter-panel exp-3d-panel overflow-hidden rounded-2xl border border-violet-100 bg-white shadow-sm"
    x-data="{
        search: '',
...
>
    <div class="border-b border-violet-100 bg-gradient-to-r from-violet-50/90 to-fuchsia-50/50 px-4 py-3">
        ...
        <p class="mt-0.5 text-xs text-slate-500">İç içe yapı; üst kategori altındaki tüm deneyleri gösterir.</p>
    </div>
    ...
    @if($activeCategory && ! empty($breadcrumbItems))
        <div class="border-b border-violet-50 px-4 py-2.5">
            @include('partials.experiment-category-breadcrumb-nav', [
    ...
    <nav class="max-h-[min(28rem,55vh)] space-y-0.5 overflow-y-auto p-2" aria-label="Deney kategori filtresi">
        <a
            href="{{ route('experiments.index') }}"
            class="exp-filter-link {{ $activeCategory ? '' : 'exp-filter-link--active' }}"
        >
            <span>Tüm deneyler</span>
            <span class="exp-filter-count {{ $activeCategory ? '' : 'exp-filter-count--active' }}">{{ $totalExperimentCount }}</span>
        </a>
