@php
    $catsById = $experimentCategories->keyBy('id');
    $orderedRows = \App\Models\ExperimentCategory::orderedFlatWithDepth($experimentCategories);
    $childCountByParent = $experimentCategories->groupBy('parent_id')->map->count();
@endphp
<div class="card p-5 md:p-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            <h2 class="text-lg font-bold text-slate-900">Deney Kategorileri</h2>
            <p class="mt-1 max-w-2xl text-sm text-slate-500">İç içe kategori açabilirsiniz (ör. Boyama → Harf Boyama). Üst kategoride ve altında deney olabilir.</p>
        </div>
        <span class="admin-blog-cat-pill admin-blog-cat-pill--count text-xs">{{ $experimentCategories->count() }} kategori</span>
    </div>

    <div class="admin-blog-cat-add mt-5">
        <p class="text-xs font-bold uppercase tracking-wider text-violet-700">Yeni kategori</p>
        <form method="post" action="{{ route('admin.experiment-categories.store') }}" class="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            @csrf
            ...
            <select name="parent_id" class="input-ui" title="Üst kategori">
                ...
                @foreach($parentSelectOptionsCreate as $opt)
                    <option value="{{ $opt['id'] }}">{{ \App\Models\ExperimentCategory::adminSelectOptionLabel($opt['depth'], $opt['name']) }}</option>
                @endforeach
            </select>
    ...
    <div
        ...
        x-data="{
            search: '',
            openIds: {},
            names: @js($experimentCategories->pluck('name', 'id')->all()),
            descriptions: @js($experimentCategories->pluck('description', 'id')->map(fn ($d) => (string) ($d ?? ''))->all()),
            matchId(id) {
    ...
    >
        <div class="admin-blog-cat-panel__head">
            <div class="min-w-0 flex-1 sm:max-w-md">
                <label for="experiment-category-filter" class="text-xs font-bold uppercase tracking-wider text-slate-600">Kategori ağacı</label>
                <input
                    id="experiment-category-filter"
                    type="search"
                ...
                <p class="mt-1.5 text-[11px] text-slate-500">
                    <span x-text="visibleIds().length"></span> / {{ $experimentCategories->count() }} gösteriliyor · Satıra tıklayarak düzenleyin
                </p>
            </div>
        ...
        <div class="admin-blog-cat-tree">
            <div class="admin-blog-cat-tree__list">
                @forelse($orderedRows as $row)
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span class="min-w-0 flex-1">
                                        ...
                                        <span class="admin-blog-cat-card__meta sm:hidden">
                                            <span class="admin-blog-cat-pill admin-blog-cat-pill--count">{{ $subtreeExperimentCounts[$cat->id] ?? 0 }} deney</span>
                                            @if($cat->is_active)
                                ...
                                >
                                    ...
                                    <form method="post" action="{{ route('admin.experiment-categories.update', $cat) }}" class="grid gap-3 md:grid-cols-2">
                                        @csrf @method('PUT')
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <select name="parent_id" class="input-ui mt-1 w-full">
                                                ...
                                                @foreach(($parentSelectOptionsForEdit[$cat->id] ?? []) as $opt)
                                                    <option value="{{ $opt['id'] }}" @selected($cat->parent_id == $opt['id'])>{{ \App\Models\ExperimentCategory::adminSelectOptionLabel($opt['depth'], $opt['name']) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div>
                                        ...
                                        <div class="flex flex-wrap items-center justify-between gap-3 md:col-span-2">
                                            <label class="inline-flex items-center gap-2 rounded-lg border border-violet-100 bg-white px-3 py-2 text-xs text-slate-600">
                                                ...
                                                Aktif (deney listesinde görünür)
                                            </label>
                                            <div class="flex flex-wrap items-center gap-2">
                                                <button type="submit" class="btn-primary px-4 py-1.5 text-xs">Kaydet</button>
                                                @if(($subtreeExperimentCounts[$cat->id] ?? 0) > 0)
                                                    <span class="text-xs text-slate-400">Silinemez ({{ $subtreeExperimentCounts[$cat->id] }} deney)</span>
                                                @endif
                                            </div>
                                        </div>
                                    </form>
                                    @if(($subtreeExperimentCounts[$cat->id] ?? 0) === 0 && ! $hasChildren)
                                        <form method="post" action="{{ route('admin.experiment-categories.destroy', $cat) }}" class="mt-3 flex justify-end border-t border-violet-100/80 pt-3">
                                            @csrf @method('DELETE')
                    ...
                    @endif
                @empty
                    <p class="rounded-xl border border-dashed border-slate-200 bg-white px-4 py-8 text-center text-sm text-slate-500">Henüz deney kategorisi yok.</p>
                @endforelse
                <p
