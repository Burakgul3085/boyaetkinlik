...
<div class="relative z-10 flex w-full items-center justify-center">
    <form method="post" action="{{ route('admin.forgot-password.submit') }}" class="w-full max-w-md rounded-3xl border border-white/20 bg-slate-900/55 p-7 text-slate-100 shadow-2xl shadow-black/45 backdrop-blur-md">
        @csrf
        <p class="inline-flex items-center rounded-full border border-indigo-300/40 bg-indigo-500/20 px-3 py-1 text-[11px] font-semibold tracking-wide text-indigo-100">Şifre sıfırlama</p>
        <h1 class="mt-2 text-3xl font-bold text-white">Admin Şifremi Unuttum</h1>
        <p class="mt-2 text-sm text-slate-300">Kayıtlı e-posta veya telefon bilgisini girin. Eşleşen hesap bulunursa yeni şifre güvenli adrese gönderilir.</p>

        @if(session('forgot_status'))
            <p class="mt-3 rounded-lg bg-emerald-100 p-2 text-sm text-emerald-700">{{ session('forgot_status') }}</p>
        @endif
        @if($errors->any())
            <p class="mt-3 rounded-lg bg-rose-100 p-2 text-sm text-rose-700">{{ $errors->first() }}</p>
        @endif

        <label class="mt-5 block text-sm font-medium text-slate-200">E-posta veya Telefon</label>
        <input type="text" name="login" value="{{ old('login') }}" required class="mt-2 w-full rounded-xl border border-slate-500/45 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-400 focus:border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-400/30" placeholder="ornek@mail.com veya 05XXXXXXXXX">

        <button class="btn-primary mt-5 w-full py-3">Yeni Şifre İste</button>
