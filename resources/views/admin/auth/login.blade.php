...
<form method="post" action="{{ route('admin.login.submit') }}" class="w-full max-w-md card p-7">
    @csrf
    <h1 class="text-2xl font-bold text-slate-900">Admin Giris</h1>
    <p class="mt-1 text-sm text-slate-500">Yonetim paneline erismek icin giris yapin.</p>
    @if(session('success'))
    ...
    <button class="btn-primary mt-5 w-full py-3">Giris Yap</button>
