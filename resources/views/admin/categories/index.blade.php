...
@section('content')
    <h1 class="text-3xl font-bold text-slate-900">Kategori Yonetimi</h1>
    <form method="post" action="{{ route('admin.categories.store') }}" class="card mt-5 grid gap-3 p-5 md:grid-cols-3">
        @csrf
        <input name="name" placeholder="Kategori adi" class="input-ui">
        <input name="slug" placeholder="Slug (opsiyonel)" class="input-ui">
        <button class="btn-primary">Ekle</button>
        <textarea name="description" placeholder="Aciklama" class="input-ui md:col-span-3"></textarea>
    </form>
            ...
            <form method="post" action="{{ route('admin.categories.update', $category) }}" class="card p-4">
                ...
                <div class="grid gap-3 md:grid-cols-3">
                    ...
                    <div class="flex gap-2">
                        <button class="btn-primary w-full">Guncelle</button>
                    </div>
                </div>
