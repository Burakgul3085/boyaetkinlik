...
@section('content')
    ...
    @if($blog->image_path)
        <section class="mx-auto mt-6 flex w-full max-w-[26rem] items-center justify-center rounded-2xl sm:max-w-[28rem] lg:max-w-[30rem]">
            <img
                src="{{ asset('storage/'.$blog->image_path) }}"
                alt="{{ $blog->title }} görseli"
                class="block max-h-56 w-full cursor-pointer rounded-2xl object-contain select-none transition-all duration-500 ease-out hover:-translate-y-2 hover:scale-110 hover:rotate-2 hover:saturate-200 hover:brightness-125 hover:contrast-110 hover:drop-shadow-2xl sm:max-h-60 md:max-h-64"
                draggable="false"
