...
@section('content')
    ...
    @if($blog->image_path)
        <section class="group mx-auto mt-6 flex w-full max-w-[26rem] items-center justify-center rounded-2xl sm:max-w-[28rem] lg:max-w-[30rem]">
            <img
                src="{{ asset('storage/'.$blog->image_path) }}"
                alt="{{ $blog->title }} görseli"
                class="block max-h-56 w-full rounded-2xl object-contain select-none transition-all duration-700 ease-out group-hover:-translate-y-2 group-hover:scale-110 group-hover:rotate-1 group-hover:saturate-150 group-hover:brightness-110 group-hover:drop-shadow-[0_18px_30px_rgba(124,58,237,0.35)] sm:max-h-60 md:max-h-64"
                draggable="false"
    ...
    <section class="group mt-6 rounded-2xl border border-violet-100 bg-white p-5 shadow-sm transition-all duration-500 hover:-translate-y-1 hover:scale-[1.01] hover:border-violet-300 hover:bg-violet-50/50 hover:shadow-[0_14px_30px_rgba(124,58,237,0.16)] md:p-6">
        <div class="prose prose-slate max-w-none leading-relaxed transition duration-500 group-hover:text-slate-900">
