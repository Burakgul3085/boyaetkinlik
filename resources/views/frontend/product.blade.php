...
@section('content')
    <div class="grid gap-6 lg:grid-cols-12">
        <div class="lg:col-span-8 card p-5 lg:p-6">
            <img src="{{ $coloringPage->cover_image_path ? asset('storage/'.$coloringPage->cover_image_path) : 'https://placehold.co/900x600/e2e8f0/334155?text=Boya+Onizleme' }}" class="w-full rounded-xl object-cover" alt="{{ $coloringPage->title }}">
            ...
            <div class="mt-6">
                {!! \App\Models\Setting::getValue('ads_product_detail') ?: '<div class="rounded-xl border border-dashed border-slate-300 p-5 text-center text-sm text-slate-500">Urun detay reklam alanı</div>' !!}
            </div>
        </div>
        ...
        <div class="lg:col-span-4 card p-5" x-data="{ open: false }">
            ...
            <p class="mt-3 text-2xl font-bold {{ $coloringPage->is_free ? 'text-emerald-600' : 'text-indigo-600' }}">
                {{ $coloringPage->is_free ? 'Ucretsiz' : number_format($coloringPage->price, 2).' TL' }}
            </p>

            @if($coloringPage->is_free)
                <a href="{{ route('products.download.free', $coloringPage) }}" class="btn-primary mt-5 w-full">Indir (PDF)</a>
                <button onclick="window.print()" class="btn-secondary mt-3 w-full">Yazdir</button>
            @else
                <button @click="open = true" class="btn-primary mt-5 w-full">Satin Al (Shopier)</button>
